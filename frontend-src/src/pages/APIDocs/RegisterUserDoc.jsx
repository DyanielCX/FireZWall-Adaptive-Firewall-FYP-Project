// ==================================================
// Register User Endpoint Content Component
// Location: /src/pages/APIDocs/RegisterUserDoc.jsx
// ==================================================

const RegisterUsersEndpointContent = ({ activeTab, setActiveTab }) => (
      ...
      <div className="bg-slate-800/50 rounded-lg p-8 border border-slate-700/50">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <code className="text-2xl font-bold text-white">/api/user/register</code>
          </div>
    </div>
        ...
        <tbody>
            <tr className="border-b border-slate-700/50">
                ...
                <td className="py-3 px-4 text-slate-400">
                    Username for the registered account
                </td>
            </tr>
            <tr className="border-b border-slate-700/50">
            ...
            <tr className="border-b border-slate-700/50">
                <td className="py-3 px-4 text-orange-400 font-mono">role</td>
                <td className="py-3 px-4 text-slate-300">string</td>
                <td className="py-3 px-4 text-slate-400">false</td>
                <td className="py-3 px-4">
                    <div className="space-y-1">
                        <div className="text-slate-400">
                            Role for the registered account 
                            <span className="text-orange-400"> (admin/dev/cybersec/user)</span>
                        </div>
                        <div className="text-slate-500 text-sm">
                            Default: <code className="text-orange-400 bg-orange-500/10 px-1.5 py-0.5 rounded">user</code>
                        </div>
                    </div>
                </td>
            </tr>
...
endpoint = "https://<server_ip>:5000/api/user/register"
...
print(r.json())`}
        ...
        <pre className="bg-slate-900/50 text-slate-300 p-4 rounded-lg overflow-x-auto">
{`const response = await fetch('https://<server_ip>:5000/api/user/register', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer <access_token>'  // Must be admin token
...
console.log(data);`}
        ...
        {activeTab === 'shell' && (
        <pre className="bg-slate-900/50 text-slate-300 p-4 rounded-lg overflow-x-auto">
{`curl -X POST https://<server_ip>:5000/api/user/register \\
...
export default RegisterUsersEndpointContent;